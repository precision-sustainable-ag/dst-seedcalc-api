...
async function getMix(){
    const PEA_WINTER = await fetch('http://localhost:3001/v2/crops/148?regions=18&context=seed_calc&regions=180')
        .then(response => {
        if (!response.ok) {
    ...
    const OATS_SPRING = await fetch('http://localhost:3001/v2/crops/23?regions=18&context=seed_calc&regions=180')
        .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    const RAPESEED = await fetch('http://localhost:3001/v2/crops/161?regions=18&context=seed_calc&regions=180')
        .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
